...
def get_noise_threshold(data_name,sim_tolerate_num, x_train, y_train, x_train_hash):
    ...
    x_random_tensor = torch.from_numpy(np.expand_dims(x_random, axis=-1)).float().permute(0, 3, 1, 2)
    from piqa import SSIM

    ssim = SSIM(n_channels=1, reduction='none').to(device)
    ...
    for i in tqdm(range(len(random_indexs))):
        ...
        img2 = torch.from_numpy(np.expand_dims(x_train[candidate_index], axis=-1)).float().permute(0, 3, 1, 2)
...
def obtain_ssim(data_name, sim_tolerate_num, x_select, x_train, y_train, x_select_hash, x_train_hash):
    ...
    select_tensor = torch.from_numpy(np.expand_dims(x_select, axis=-1)).float().permute(0, 3, 1, 2)
    ...
    for line_number, single_select in tqdm(enumerate(select_tensor)):  # 测试用例的原始序列
        ...
        for label, indexs in class_matrix.items():
            ...
            img2 = torch.from_numpy(np.expand_dims(img2, axis=-1)).float().permute(0, 3, 1, 2)
            img1, img2 = torch.broadcast_tensors(single_select, img2)
            if torch.cuda.is_available():
                img1 = img1.to(device)
                img2 = img2.to(device)
...
def ob_sus_correct(sim_tolerate_num, ssim_datas,
    ...
    y = torch.tensor(train_pro, dtype=torch.float32).unsqueeze(0).to()
    chuck = 200
