...
def get_selection_information_new(file_path, model, lstm_classifier, dense_model, wrapper_path, w2v_path, time_steps):
    ...
    final = [mymethod, diss]
    return feature_mat, weight_state, unique_index_arr_id, np.array(stellar_bscov), np.array(stellar_btcov), np.array(
        rnntest_sc), \
        np.array(nc_cov), np.array(nc_cam), np.array(
        rnntest_sc_cam), final, trend_set, right, fault_types, deepgini_set, maxp_set
...
def get_right(file_path, model, lstm_classifier, dense_model, wrapper_path, w2v_path, time_steps):
    if file_path.split(".")[-1] == "npz":
        with np.load(file_path, allow_pickle=True) as f:
            X, Y = f['X'], f['Y']
    if file_path.split(".")[-1] == "csv" and "snips" in file_path.split(".")[-2]:
...
from selection_method.selection_utils import get_rank_func

# rts only used on image
def rts_selection_rank(file_path, model):
    if file_path.split(".")[-1] == "npz":
        with np.load(file_path, allow_pickle=True) as f:
            X, Y = f['X'], f['Y']
    else:
    ...
    if file_path.split(".")[-1] == "npz" and "svhn" in file_path.split(".")[-2]:
        x_train, y_train = get_train_data("svhn")
        div_rank = get_rank_func(x_train, y_train, model, dataname='svhn', x_s=X, nb_classes=10)
    elif file_path.split(".")[-1] == "npz" and "mnist" in file_path.split(".")[-2]:
        x_train, y_train = get_train_data("mnist")
        div_rank = get_rank_func(x_train, y_train, model, dataname='mnist', x_s=X, nb_classes=10)
    else:
        x_train, y_train = get_train_data("fashion")
        div_rank = get_rank_func(x_train, y_train, model, dataname='fashion', x_s=X, nb_classes=10)
    print(div_rank)

    return div_rank

def rts_selection(rank, length, selected_num):
    selected = np.zeros(length)
    selected[rank[:selected_num]] = 1
    return selected
...
def get_gan_rank(file_path):
    ...
    if file_path.split(".")[-1] == "npz" and "svhn" in file_path.split(".")[-2]:
        gan = DCGAN("SVHN")
        div_rank = gan.get_fidelity_list(X)
    elif file_path.split(".")[-1] == "npz" and "mnist" in file_path.split(".")[-2]:
        gan = DCGAN("MNIST")
        div_rank = gan.get_fidelity_list(X)
    else:
...
def gan_selection(rank, length, selected_num):
    selected = np.zeros(length)
    selected[rank[:selected_num]] = 1
    return selected