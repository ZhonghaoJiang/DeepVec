...
class SnipsLSTMClassifier:
    ...
    def train_model_(self, save_path):
        self.create_model()
        checkpoint = ModelCheckpoint(filepath=os.path.join(save_path, "snips_lstm_ori.h5"),
                                     monitor='val_acc', verbose=1, save_best_only=True, mode='auto')
        self.model.fit(self.X_train, self.Y_train, epochs=self.n_epochs, batch_size=self.batch_size,
                       validation_data=(self.X_test, self.Y_test), callbacks=[checkpoint])
        os.makedirs(save_path, exist_ok=True)
        self.model.save(os.path.join(save_path, "snips_lstm_ori.h5"))
        print(self.model.evaluate(self.X_test, self.Y_test))
    ...
    def dau_train(self, save_path):
        ...
        checkpoint = ModelCheckpoint(filepath=os.path.join(save_path, "snips_lstm_aug.h5"),
                                     monitor='val_acc', verbose=1, save_best_only=True, mode='auto')
        self.model.fit(self.X_train, self.Y_train, epochs=self.n_epochs, batch_size=self.batch_size,
                       validation_data=(self.X_test, self.Y_test), callbacks=[checkpoint])
        os.makedirs(save_path, exist_ok=True)
        self.model.save(os.path.join(save_path, "snips_lstm_aug.h5"))
