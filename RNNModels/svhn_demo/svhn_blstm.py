...
from keras.layers import Input, Lambda, Dense, Dropout, Bidirectional, LSTM
...
class SvhnBLSTMClassifier:
    ...
    def train(self, save_path):
        ...
        checkpoint = ModelCheckpoint(filepath=os.path.join(save_path, "svhn_blstm.h5"), monitor='val_acc', mode='auto',
                                     save_best_only="True")
        ...
        self.model.save(os.path.join(save_path, "svhn_blstm.h5"))
    ...
    def train_(self, save_path):
        ...
        checkpoint = ModelCheckpoint(filepath=os.path.join(save_path, "svhn_blstm_ori.h5"), monitor='val_acc', mode='auto',
                                     save_best_only=True)
        self.model.fit(x_train, y_train, validation_data=(x_test, y_test),
                       batch_size=self.batch_size, epochs=self.n_epochs, shuffle=False, callbacks=[checkpoint])

        os.makedirs(save_path, exist_ok=True)
        self.model.save(os.path.join(save_path, "svhn_blstm_ori.h5"))
    ...
    def dau_train(self, save_path, x_dau_train, x_dau_test, y_dau_train, y_dau_test):
        ...
        checkpoint = ModelCheckpoint(filepath=os.path.join(save_path, "svhn_blstm_dau.h5"), monitor='val_acc',
                                     mode='auto',
                                     save_best_only=True)
        self.model.fit(x_train, y_train, validation_data=(x_test, y_test),
                       batch_size=self.batch_size, epochs=self.n_epochs, shuffle=False, callbacks=[checkpoint])
...
if __name__ == "__main__":
    parse = argparse.ArgumentParser("Train the BLSTM model on SVHN dataset.")
