...
class SvhnGRUClassifier:
    def __init__(self):
        # Classifier
        self.time_steps = 32  # timesteps to unroll
        self.n_units = 128  # hidden GRU units
        self.n_inputs = 32  # rows of 32 pixels (an svhn img is 32x32)
        self.n_classes = 10  # svhn classes/labels (0-9)
    ...
    def create_model(self):
        ...
        gru = GRU(self.n_units, return_sequences=True, dropout=0.3, name='gru')(input)
    ...
    def load_hidden_state_model(self, model_path):
        ...
        """
        input = Input(shape=(self.time_steps, self.n_inputs))
        gru = GRU(self.n_units, input_shape=(self.time_steps, self.n_inputs), return_sequences=True,
                    dropout=0.3, name="gru")(input)
        last_timestep = Lambda(lambda x: x[:, -1, :])(gru)
        dense1 = Dense(64, activation="relu", name='dense1')(last_timestep)
        dropout = Dropout(0.4, name='drop')(dense1)
        dense2 = Dense(self.n_classes, activation="softmax", name='dense2')(dropout)
    ...
    def train(self, save_path):
        ...
        x_train = self.input_preprocess(x_train[:-7000])
        x_test = self.input_preprocess(x_test)

        y_test = keras.utils.to_categorical(y_test, num_classes=10)
        y_train = keras.utils.to_categorical(y_train[:-7000], num_classes=10)

        checkpoint = ModelCheckpoint(filepath=os.path.join(save_path, "svhn_gru.h5"),
        ...
        self.model.save(os.path.join(save_path, "svhn_gru.h5"))
    ...
    def train_(self, save_path):
        ...
        checkpoint = ModelCheckpoint(filepath=os.path.join(save_path, "svhn_gru_ori.h5"),
                                     monitor='val_acc', mode='auto', save_best_only=True)
        self.model.fit(x_train, y_train, validation_data=(x_test, y_test),
                       batch_size=self.batch_size, epochs=self.n_epochs, shuffle=False, callbacks=[checkpoint])

        os.makedirs(save_path, exist_ok=True)
        self.model.save(os.path.join(save_path, "svhn_gru_ori.h5"))
    ...
    def dau_train(self, save_path, x_dau_train, x_dau_test, y_dau_train, y_dau_test):
        ...
        checkpoint = ModelCheckpoint(filepath=os.path.join(save_path, "svhn_gru_dau.h5"), monitor='val_acc',
                                     mode='auto',
                                     save_best_only=True)
        self.model.fit(x_train, y_train, validation_data=(x_test, y_test),
                       batch_size=self.batch_size, epochs=self.n_epochs, shuffle=False, callbacks=[checkpoint])
        ...
        os.makedirs(save_path, exist_ok=True)
        self.model.save(os.path.join(save_path, "svhn_gru_dau.h5"))
    ...
    def retrain(self, X_selected, Y_selected, X_val, Y_val, save_path):
        self.create_model()
        x_train, y_train, x_test, y_test = self.load_svhn_data()
        Xa_train = np.concatenate([X_selected, x_train[:-7000]])
        Ya_train = np.concatenate([Y_selected, y_train[:-7000]])
    ...
    def evaluate_retrain(self, retrain_model_path, ori_model_path, x_val, y_val):
        ...
        return retrain_acc, retrain_acc - ori_acc  # retrain acc, imp acc

    def input_preprocess(self, data):
        data = data.reshape(data.shape[0], self.n_inputs, self.n_inputs, 3)
...
def train_model():
    save_path = "./models"
    gru_classifier = SvhnGRUClassifier()
    # train an rnn model
    gru_classifier.create_model()
    gru_classifier.train(save_path)
...
def train_model_ori():
    ...
    # train an rnn model
    gru_classifier.create_model()
    gru_classifier.train_(save_path)
...
if __name__ == "__main__":
    parse = argparse.ArgumentParser("Train the GRU model on SVHN dataset.")
