...
class MnistGRUClassifier:
    ...
    def load_hidden_state_model(self, model_path):
        ...
        """
        input = Input(shape=(self.time_steps, self.n_inputs))
        lstm = GRU(self.n_units, input_shape=(self.time_steps, self.n_inputs), return_sequences=True,
                    dropout=0.3, name="lstm")(input)
        last_timestep = Lambda(lambda x: x[:, -1, :])(lstm)
        dense1 = Dense(64, activation="relu", name='dense1')(last_timestep)
        dropout = Dropout(0.4, name='drop')(dense1)
        dense2 = Dense(self.n_classes, activation="softmax", name='dense2')(dropout)
    ...
    def train(self, save_path):
        ...
        checkpoint = ModelCheckpoint(filepath=os.path.join(save_path, "mnist_gru.h5"),
        ...
        self.model.save(os.path.join(save_path, "mnist_gru.h5"))
    ...
    def train_(self, save_path):
        ...
        os.makedirs(save_path, exist_ok=True)
        self.model.save(os.path.join(save_path, "mnist_gru_ori.h5"))
    ...
    def dau_train(self, save_path, x_dau_train, x_dau_test, y_dau_train, y_dau_test):
        ...
        checkpoint = ModelCheckpoint(filepath=os.path.join(save_path, "mnist_gru_dau.h5"), monitor='val_acc',
                                     mode='auto',
                                     save_best_only=True)
        self.model.fit(x_train, y_train, validation_data=(x_test, y_test),
                       batch_size=self.batch_size, epochs=self.n_epochs, shuffle=False, callbacks=[checkpoint])
        ...
        os.makedirs(save_path, exist_ok=True)
        self.model.save(os.path.join(save_path, "mnist_gru_dau.h5"))
...
if __name__ == "__main__":
    parse = argparse.ArgumentParser("Train the GRU model on MNIST dataset.")
