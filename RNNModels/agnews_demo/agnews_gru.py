...
def process_aug_data(train_data_path, test_data_path, save_path):
    ...
    # Split dataset
    X_train = X_pad[:len(df1)]  # rq1,2:  [0:120000]  rq3: [6000:120000]
    X_test = X_pad[len(df1):]
    Y_train = Y[:len(df1)]  # 0:120000
    Y_test = Y[len(df1):]
...
class AgnewsGRUClassifier:
    ...
    def train_model(self, save_path):
        self.create_model()
        checkpoint = ModelCheckpoint(filepath=os.path.join(save_path, "agnews_gru.h5"),
        ...
        self.model.save(os.path.join(save_path, "agnews_gru.h5"))
        print(self.model.evaluate(self.X_test, self.Y_test))

    def train_model_(self, save_path):
        self.create_model()
        checkpoint = ModelCheckpoint(filepath=os.path.join(save_path, "agnews_gru_ori.h5"),
                                     monitor='val_acc', verbose=1, save_best_only=True, mode='auto')
        self.model.fit(self.X_train, self.Y_train, epochs=self.n_epochs, batch_size=self.batch_size,
                       validation_data=(self.X_test, self.Y_test), callbacks=[checkpoint])
        os.makedirs(save_path, exist_ok=True)
        self.model.save(os.path.join(save_path, "agnews_gru_ori.h5"))
        print(self.model.evaluate(self.X_test, self.Y_test))
    ...
    def dau_train(self, save_path):
        ...
        checkpoint = ModelCheckpoint(filepath=os.path.join(save_path, "agnews_gru_aug.h5"),
                                     monitor='val_acc', verbose=1, save_best_only=True, mode='auto')
        self.model.fit(self.X_train, self.Y_train, epochs=self.n_epochs, batch_size=self.batch_size,
                       validation_data=(self.X_test, self.Y_test), callbacks=[checkpoint])
        os.makedirs(save_path, exist_ok=True)
        self.model.save(os.path.join(save_path, "agnews_gru_aug.h5"))
