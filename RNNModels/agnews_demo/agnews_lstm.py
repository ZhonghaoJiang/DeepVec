...
from keras.layers import Dense, LSTM, Embedding, Dropout, Input, Lambda
...
def process_data(data_path, save_path):
    ...
    # Split dataset
    X_train = X_pad[6000:120000]  # rq1,2:  [0:120000]  rq3: [6000:120000]
    X_test = X_pad[120000:]
    Y_train = Y[6000:120000]  # 0:120000
    Y_test = Y[120000:]
...
class AGNewsLSTMClassifier:
    ...
    def create_model(self):
        ...
        lstm = LSTM(self.n_units, return_sequences=True, dropout=0.5, name='lstm')(embedding)
    ...
    def dau_train(self, save_path):
        ...
        checkpoint = ModelCheckpoint(filepath=os.path.join(save_path, "agnews_lstm_aug.h5"),
                                     monitor='val_acc', verbose=1, save_best_only=True, mode='auto')
        self.model.fit(self.X_train, self.Y_train, epochs=self.n_epochs, batch_size=self.batch_size,
                       validation_data=(self.X_test, self.Y_test), callbacks=[checkpoint])
        os.makedirs(save_path, exist_ok=True)
        self.model.save(os.path.join(save_path, "agnews_lstm_aug.h5"))
