...
from selection_tools import get_selection_information, get_selection_information_new, ats_selection, ats_selection_rank, \
    rts_selection_rank, get_right, rts_selection
import keras
import datetime
import sys
import os
...
if __name__ == '__main__':
    ...
    for file in files:
        ...
        rts_rank = rts_selection_rank(file_path, model)
        right = get_right(
        ...
        for pre in pre_li:
            ...
            rts_bdr[pre].append(rts_P)



    result_dict = {'RTS_w10':rts_bdr[10],'RTS_w20':rts_bdr[20]}

    print(result_dict)
    df = pd.DataFrame(result_dict)
    os.makedirs("./exp_results/rq-rts-1", exist_ok=True)
    df.to_csv("./exp_results/rq-rts-1/rq-rts-1_{}_{}.csv".format(args.dataset, args.model_type))

    print("Finished! The results are saved in: [./exp_results/rq-rts-1/rq-rts-1_{}_{}.csv]".format(args.dataset, args.model_type))
