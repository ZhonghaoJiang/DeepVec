...
def line_chart():
    ...
    # 选择你需要处理的列的名称（第2列到最后一列）
    columns = data.columns[2:]

    for col in columns:
        # 提取括号内的百分比
        data[col] = data[col].str.extract('\((.*?)%\)', expand=False)

        # 将百分比转化为小数
        data[col] = data[col].astype(float)

    mix = [data['mymethod_selected_mix'],data['random_selected_mix'], data['state_w_selected_mix'], data['cov_selected_mix'],
    ...
    aug = [data['mymethod_selected_aug'],data['random_selected_aug'], data['state_w_selected_aug'],
    ...
    labels = ['NewMethod', 'Random', 'DeepState', 'RNNTest-HSCov(CAM)', 'DeepStellar-BSCov(CTM)', 'DeepStellar-BTCov(CTM)',
