...
def line_chart(data_path):
    ...
    datas = [data['random']*100, data['state']*100, data['RNNTestcov']*100, data['Stellarbscov']*100,
             ...
             data['nc_cam']*100, data['my']*100]
    labels = ['Random', 'DeepState', 'RNNTest-HSCov(CAM)', 'DeepStellar-BSCov(CTM)', 'DeepStellar-BTCov(CTM)',
              'testRNN-SC(CTM)', 'testRNN-SC(CAM)', 'NC(CTM)', 'NC(CAM)',"DeepVec"]  # labels
    x = np.arange(1, 41)
    ...
    plt.plot(x, datas[1][1:41], label=labels[1], linewidth=4)
    for i in range(2, 9):
        plt.plot(x, datas[i][1:41], label=labels[i])
    plt.plot(x, datas[9][1:41], label=labels[9], color='red', linewidth=4)
