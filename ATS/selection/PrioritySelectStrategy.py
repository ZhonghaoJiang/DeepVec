...
class PrioritySelectStrategy(AbsSelectStrategy):
    ...
    def get_priority_sequence(self, Tx, Ty, n, M, dense_model, base_path=None, prefix=None, is_save_ps=False, th=0.001, dataset='mnist'):
    ...
    # no target_size
    def get_max_coverage_sequence(self, Tx, Ty, n, M, dense_model, base_path=None, use_add=True, is_save_ps=False, prefix="",
                                  th=0.001, dataset='mnist'):
        df = None
        csv_data = {}
        c_arr = []  # cov
        ...
        for i in tqdm(range(n)):
            ...
            if dataset == 'mnist' or dataset == 'fashion':
                ...
                Tx_i = Tx_i
            lstm_out = M.predict(Tx_i)[1]
            Tx_prob_matrixc = np.zeros((lstm_out.shape[0], dense_model.predict(lstm_out[:, 0, :]).shape[1]))
            weight_sum = 0
            for t in range(lstm_out.shape[1] - 1):
                weight = (t+1) ** 0.1
                weight_sum += weight
                dense_array = lambda x: x[:, t, :]
                tmp = dense_model.predict(np.array(dense_array(lstm_out)))
                Tx_prob_matrixc += weight * tmp
            Tx_prob_matrixc /= weight_sum
