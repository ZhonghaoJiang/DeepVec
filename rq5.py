...
if __name__ == '__main__':
    ...
    for pre in pre_li:
        ...
        state_w_runtime = time.time() - start_time
        ...
        deepgini_runtime = time.time() - start_time

        start_time = time.time()
        kmedoids_selected = kmedoids_selection(np.array(feature_mat), total_num, select_num)
        kmedoids_runtime = time.time() - start_time

        start_time = time.time()
        mmdcritic_selected = mmdcritic_selection(np.array(feature_mat), total_num, select_num)
        mmdcritic_runtime = time.time() - start_time

        start_time = time.time()
        maxp_selected = maxP_selection(np.array(maxp), total_num, select_num)
        maxp_runtime = time.time() - start_time

        start_time = time.time()
        rank = ats_selection_rank(file_path, w2v_path, model,dense_model)
        ats_selected = ats_selection(rank, total_num, select_num)
        ats_runtime = time.time() - start_time

        # 将运行时间写入后面的列表
        state_w_in.append(state_w_runtime)
        ran_in.append(random_runtime)
        RNNTestcov_in.append(cov_runtime)
        Stellarbscov_in.append(bscov_runtime)
        Stellarbtcov_in.append(btcov_runtime)
        sc_ctm_in.append(sc_ctm_runtime)
        sc_cam_in.append(sc_cam_runtime)
        nc_ctm_in.append(nc_ctm_runtime)
        nc_cam_in.append(nc_cam_runtime)
        my_in.append(mymethod_runtime)
        deepgini_in.append(deepgini_runtime)
        kmedoids_in.append(kmedoids_runtime)
        mmdcritic_in.append(mmdcritic_runtime)
        maxp_in.append(maxp_runtime)
        ats_in.append(ats_runtime)
    ...
    os.makedirs("./exp_results/rq5", exist_ok=True)
    df.to_csv("./exp_results/rq5/rq5_{}_{}.csv".format(args.dataset, args.model_type))

    print("Finished! The results are saved in: [./exp_results/rq5/rq5_{}_{}.csv]".format(args.dataset, args.model_type))
